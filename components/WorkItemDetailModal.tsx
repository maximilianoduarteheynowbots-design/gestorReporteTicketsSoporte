                ...
                {comments.map(comment => (
                    <li key={comment.id} className="bg-light-bg dark:bg-dark-bg p-3 rounded-md">
                        <div className="flex justify-between items-center text-xs mb-1">
                            ...
                            <p className="text-gray-500 dark:text-gray-400">{formatDate(comment.createdDate)}</p>
                        </div>
                ...
                <main className="p-6 overflow-y-auto">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-6 text-sm">
                        ...
                        {fields['System.WorkItemType'].toLowerCase() === 'linea' && (
                            <>
                                <DetailPill label="Fecha (Línea)" value={formatDate(fields['Custom.Fechalinea'])} />
                                <DetailPill label="Horas" value={fields['Custom.Horas'] ?? 'N/A'} />
                            </>
                        )}
                        <DetailPill label="Fecha de Creación" value={formatDate(fields['System.CreatedDate'])} />
                        <DetailPill label="Última Actualización" value={formatDate(fields['System.ChangedDate'])} />
                    </div>
