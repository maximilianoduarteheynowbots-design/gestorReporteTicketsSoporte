...
const NoItemsMessage: React.FC<{ totalItemsCount: number }> = ({ totalItemsCount }) => {
    const message = totalItemsCount > 0 
        ? "Ningún elemento coincide con los filtros actuales." 
...
export const WorkItemViewer: React.FC<WorkItemViewerProps> = ({
    ...
    const showFilters = !isLoading && (availableTypes.length > 0 || availableStates.length > 0 || availableAssignees.length > 0);
            ...
            {isLoading ? (
                ...
                <>
                    {workItems.length === 0 && !error ? (
                         <NoItemsMessage totalItemsCount={totalItemsCount} />
                    ) : (
                        <WorkItemList 
                            items={workItems} 
